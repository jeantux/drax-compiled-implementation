...
static int dx_x86_64_return(dline_cmd* e) {
  df_asm_gen(dxasm_cmd_table(e->op));
  df_asm_gen(FL);
...
static int dx_x86_64_jump(dline_cmd* e) {
  df_asm_gen(dxasm_cmd_table(e->op));
...
int get_asm_code(dline_cmd* v) {
  switch (v->op) {
    ...
    /* Jump Section */
    case DOP_CALL:
    case DOP_JUMP: return dx_x86_64_jump(v);
    ...
    case DOP_RETURN: return dx_x86_64_return(v);
